...
# to perform planned contrasts & post-hocs /!\ load before tidyverse to prevent masking its 'select' function
library(multcomp)
...
```{r}
lme.fail = lmer(rt ~ group*session+ (1+session|participant)+ (1+group|trial), data = data) # failed
```
...
```{r}
lme.singluar = lmer(rt ~ group*session+ (1|participant)+ (1+group|trial), data = data)
```
...
```{r}
lme.full = lmer(rt ~ group*session + (1+session|participant) + (1 + group*session|item), data = data)
```
...
```{r, eval = FALSE}
lme = lmer(rt ~ group*session + (1+session|participant) + (1|item) + (0+group*session|item), data = data) # singular
lme = lmer(rt ~ group*session + (1+session|participant) + (1+group+session|item), data = data) # singular
lme = lmer(rt ~ group*session + (1+session|participant) + (1|item) + (0+group+session|item), data = data) # singular
lme = lmer(rt ~ group*session + (1+session|participant) + (1+group|item), data = data) # singular
lme = lmer(rt ~ group*session + (1+session|participant) + (1|item) +(0+group|item), data = data) # singular
```
...
```{r}
lme.trainingcontroledforschools = lmer(rt ~ session + (1|participant) +(1+session|school), data = data.school) 
...
```{r}
lme.traininginteractionschools = lmer(rt ~ school*session + (1|participant), data = data.school) 
...
lme.conv.contrast = lmer(rt ~ group.contrast*session.contrast + (1+session|participant) + (1|item), data = data.contrast) 
...
```{r}
lme.int = lmer(rt ~ group*session - group:session+ (1+session|participant) + (1|item), data = data) 
lme.conv = lmer(rt ~ group*session + (1+session|participant) + (1|item), data = data) 
...
```{r}
lme.int = lmer(rt ~ group*session - group+ (1+session|participant) + (1|item), data = data) 
lme.conv = lmer(rt ~ group*session + (1+session|participant) + (1|item), data = data) 
anova(lme.conv, lme.int)
```
